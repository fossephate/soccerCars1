...
gameServer.prototype.makeVehicle = function() {
	var car = {};
	...
	car.parts.chassis.body = new CANNON.Body({mass:150.0});
	car.parts.chassis.body.addShape(chassisShape);
	...
	var options = {
		...
		customSlidingRotationalSpeed: -90,//-30
...
function node(type, owner) {
	...
	if(this.type == "player"){
		...
		//this.tObject.parts.chassis.body.position.x = 0;
...
node.prototype.move = function() {
  ...
  if(typeof keys !== "undefined") {
    ...
    if(keys.indexOf("space") > -1 && this.tObject.vehicle.chassisBody.angularVelocity.z > -5  && this.tObject.vehicle.chassisBody.position.z < 1.3 && this.temp.jumps < 1) {
			this.temp.jumps = 1;
			this.tObject.vehicle.chassisBody.applyLocalImpulse(new CANNON.Vec3(0, 0, 200), new CANNON.Vec3(0, 0, 0));
    }
		...
		if(keys.indexOf("space") > -1 && this.temp.jumps == 1 && this.tObject.vehicle.chassisBody.position.z > 3.3) {
			...
			this.temp.jumps = 2;
			if(keys.indexOf("w") > -1) {
				this.tObject.vehicle.chassisBody.applyLocalImpulse(new CANNON.Vec3(0, 0, 55), new CANNON.Vec3(20, 0, 0));
				this.tObject.vehicle.chassisBody.applyLocalImpulse(new CANNON.Vec3(-800, 0, 0), new CANNON.Vec3(0, 0, 0));
			} else if(keys.indexOf("s") > -1) {
				this.tObject.vehicle.chassisBody.applyLocalImpulse(new CANNON.Vec3(0, 0, 55), new CANNON.Vec3(-20, 0, 0));
				this.tObject.vehicle.chassisBody.applyLocalImpulse(new CANNON.Vec3(800, 0, 0), new CANNON.Vec3(0, 0, 0));
			} else if(keys.indexOf("a") > -1) {
				this.tObject.vehicle.chassisBody.applyLocalImpulse(new CANNON.Vec3(0, 0, 55), new CANNON.Vec3(0, 10, 0));
				this.tObject.vehicle.chassisBody.applyLocalImpulse(new CANNON.Vec3(0, -800, 0), new CANNON.Vec3(0, 0, 0));
			} else if(keys.indexOf("d") > -1) {
				this.tObject.vehicle.chassisBody.applyLocalImpulse(new CANNON.Vec3(0, 0, 55), new CANNON.Vec3(0, -10, 0));
				this.tObject.vehicle.chassisBody.applyLocalImpulse(new CANNON.Vec3(0, 800, 0), new CANNON.Vec3(0, 0, 0));
			} else {
				this.tObject.vehicle.chassisBody.applyLocalImpulse(new CANNON.Vec3(0, 0, 1200), new CANNON.Vec3(0, 0, 0));
			}
    }
		...
		if(keys.indexOf("shift") > -1 ) {
			this.tObject.vehicle.chassisBody.applyLocalImpulse(new CANNON.Vec3(-50, 0, 0), new CANNON.Vec3(100, 0, 0));
			this.temp.airTurnForce = 50;
		} else if(this.temp.airTurnForce == 50) {
