...
gameServer.prototype.initScene = function() {
	...
	var wallShape = new CANNON.Box(new CANNON.Vec3(200, 1, 200));
	var wallBody = new CANNON.Body({
		mass: 0
	});
	wallBody.addShape(wallShape);
	wallBody.position.set(0, 200, 100);
	this.createPhysicsObject(wallBody);
  
  
	wallShape = new CANNON.Box(new CANNON.Vec3(200, 1, 200));
	...
	wallShape = new CANNON.Box(new CANNON.Vec3(1, 200, 200));
	wallBody = new CANNON.Body({
		mass: 0
	});
	wallBody.addShape(wallShape);
	wallBody.position.set(200, 0, 100);
	this.createPhysicsObject(wallBody);
...
function node(type, owner, clapos, username) {
	this.type = type;
	if(typeof owner != "undefined") {
		...
		this.username = "test"+Math.floor(Math.random()*5000);
	}
    ...
    if(keys.indexOf("space") > -1 && this.tObject.vehicle.chassisBody.angularVelocity.z > -5  && this.tObject.vehicle.chassisBody.position.z < 1.3 && this.temp.jumps < 1) {
		...
		if(keys.indexOf("space") > -1 && this.temp.jumps == 1 && this.tObject.vehicle.chassisBody.position.z > 3.3) {
			...
			if(keys.indexOf("w") > -1) {
				...
				this.tObject.vehicle.chassisBody.applyLocalImpulse(new CANNON.Vec3(0, 0, this.temp.jumpRollForce/2), new CANNON.Vec3(0, 10, 0));
