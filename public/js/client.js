$(function() {
	...
	socket = io('https://fosse.co', {path: '/8100/socket.io'});
		...
		for(var i = 0; i < world1.game.visiblePlayersData.length; i++) {
			...
			if(vpd[i].type == "player") {
				...
				if(vpd[i].username == world1.game.player.username) {
					...
					//playerObject.parts.chassis.body.position.lerp(vpd[i].position, 0.4);
					//playerObject.parts.chassis.body.quaternion.slerp(vpd[i].quaternion, 0.4);
					//playerObject.parts.chassis.body.velocity.lerp(vpd[i].velocity, 0.4);
					
					
					/*var chBody = playerObject.parts.chassis.body;
					var pDiff = new THREE.Vector3().subVectors(vpd[i].position, chBody.position);
					var vDiff = new THREE.Vector3().subVectors(vpd[i].velocity, chBody.velocity);
					pDiff.multiplyScalar(1);
					vDiff.multiplyScalar(1);
					chBody.position.add(pDiff);
					chBody.velocity.add(vDiff);*/
				...
				if(typeof vp[vpd[i].username] == "undefined" && typeof car != "undefined") {
					...
					//tObject.position.lerp(vpd[i].position, 0.4);
					//tObject.quaternion.slerp(vpd[i].quaternion, 0.4);
	...
	function followObject(world, obj, cam, options) {
		...
		*/
		...
		ideal.position.x = tPos.x + Math.cos(tRot.z+targetSet.rotateOffset.z)*10;
		ideal.position.y = tPos.y + Math.sin(tRot.z+targetSet.rotateOffset.z)*10;
		//ideal.position.z = tPos.z + targetSet.camPos.z;
		ideal.position.z = tPos.z + Math.sin(-tRot.y + targetSet.rotateOffset.y + Math.PI/6)*10;
	...
	//emitter.position.value = world1.game.player.tObject.parts.chassis.body.position;
	...
	function boost() {
		...
		//explosion.cloneNode(true).play();
		/*particleGroup.mesh.position = new THREE.Vector3().copy(world1.game.player.tObject.parts.chassis.body.position);
		emitter.position.value = new THREE.Vector3().copy(world1.game.player.tObject.parts.chassis.body.position);
		emitter.position.value.z += 3;
	...
	function gameLoop(world) {
		if(world.game.connected) {
			...
			if(input.key.up) {
				options.rotateOffset.y = Math.PI/6;
				//cube.rotation.x += 0.1;
